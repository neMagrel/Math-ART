"""Teacher demo for the Math Art Generator masterclass."""
...
# Always save beside the project, even when an IDE uses another working directory.
...
def position(t: float, phase: float):
    """Return a point on the current Lissajous curve."""
    x = CENTER_X + math.sin(a * t + phase) * radius_x
    y = CENTER_Y + math.sin(b * t) * radius_y
    return int(x), int(y)


def save_png(surface):
    """Save the clean art surface as a wallpaper image."""
...
def save_gif(frames):
    """Save captured Pygame surfaces as an animated GIF."""
...
def randomize():
    """Choose a new combination of visible parameters."""
...
def draw_interface(screen, font, small_font):
    """Draw a compact projector-friendly status and help panel."""
    title = font.render(
    ...
    if show_help:
        ...
        panel = pygame.Surface((330, 290), pygame.SRCALPHA)
        panel.fill((0, 0, 0, 135))
        screen.blit(panel, (20, HEIGHT - 310))
...
def main():
    global a, b, speed, particle_count
    global time_value, hue_shift
    global show_help, paused
    global recording, recorded_frames, frame_counter

    pygame.init()
    pygame.display.set_caption("Math Art Generator - teacher demo")
    ...
    while running:
        ...
        for i in range(particle_count):
            phase = math.tau / particle_count * i
            x, y = position(time_value, phase)
            color = hsv_color(hue_shift + i / particle_count)

            previous = previous_points[i]
